function LineChart(elementId) {
	...
	var onzoom = function(changeDomain) {
		...
		if (!changeDomain) {
			...
			start = d0 - (t.x / (clientWidth * t.k)) * (d1 - d0);
	...
	var zoom = d3.zoom().scaleExtent([1, 24]).translateExtent([[0, 0], [clientWidth, 0]]).on("zoom", onzoom);
	
	svg.call(zoom);

	var onresize = function() {
		clientWidth = element.clientWidth;
		zoom.translateExtent([[0, 0], [clientWidth, 0]]);
		onzoom(true);
	}
